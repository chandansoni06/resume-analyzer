import streamlit as st
st.set_page_config(
    page_title="Resume Checker",  # Yahan jo naam likhoge, wo browser ke upar tab par dikhega
    page_icon="🎯"                    # Yahan koi bhi emoji daal sakte ho (jaise 📄 ya 🚀)
)
import pdfplumber
from docx import Document
from google import genai

# ----------------------------------------------------
# APNI GEMINI API KEY YAHAN FIX KAR DO
# ----------------------------------------------------
GEMINI_API_KEY = st.secrets["GEMINI_API_KEY"]

# Page Config
st.set_page_config(page_title="AI Resume Analyzer", page_icon="🤖", layout="wide")

st.title("🤖 Resume Checker")
st.markdown("Upload your resume and get an instant professional analysis!")

# Main Interface
col1, col2 = st.columns(2)

with col1:
    uploaded_file = st.file_uploader("Upload Resume (PDF or DOCX)", type=["pdf", "docx"])

with col2:
    job_description = st.text_area("Target Job Description (Optional)", placeholder="Paste target job description here...")

# Helper Functions
def extract_text_from_pdf(file):
    text = ""
    with pdfplumber.open(file) as pdf:
        for page in pdf.pages:
            text += page.extract_text() or ""
    return text

def extract_text_from_docx(file):
    doc = Document(file)
    return "\n".join([para.text for para in doc.paragraphs])

# Analyze Button
if st.button("🚀 Analyze Resume", type="primary"):
    if GEMINI_API_KEY == "GEMINI_KEY_YAHAN_DALO" or not GEMINI_API_KEY:
        st.error("Developer Error: Please update your Gemini API key inside the code script.")
    elif uploaded_file is None:
        st.warning("Please upload a resume file first.")
    else:
        with st.spinner("Extracting text from resume..."):
            if uploaded_file.name.endswith(".pdf"):
                resume_text = extract_text_from_pdf(uploaded_file)
            else:
                resume_text = extract_text_from_docx(uploaded_file)

        if resume_text.strip() == "":
            st.error("Could not read text from the resume.")
        else:
            with st.spinner("AI is analyzing your resume... Please wait."):
                try:
                    client = genai.Client(api_key=GEMINI_API_KEY)
                    
                    prompt = f"""
                    You are an expert ATS and Career Coach. Analyze the given resume and provide a detailed report strictly in English:
                    
                    Resume Text: {resume_text}
                    Job Description: {job_description if job_description else "N/A"}
                    
                    Please structure your response with:
                    1. ATS Score (0-100)
                    2. Strengths
                    3. Weaknesses/Areas for Improvement
                    4. Missing Keywords
                    5. Career Recommendations
                    """
                    
                    response = client.models.generate_content(
                        model='gemini-3.6-flash',
                        contents=prompt
                    )
                    
                    st.markdown("---")
                    st.markdown(response.text)
                    
                except Exception as e:
                    st.error(f"An error occurred: {e}")